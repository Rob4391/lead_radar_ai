...
- `POST /leads/:id/outreach` generates a cold email / LinkedIn message / WhatsApp message for a lead. Defaults to **Ollama** (free, local — run `ollama serve` and pull a model first; `OLLAMA_BASE_URL` default `http://localhost:11434`, `OLLAMA_MODEL` default `qwen2.5:1.5b`). Set `LLM_PROVIDER=anthropic` plus `ANTHROPIC_API_KEY` (optional `ANTHROPIC_MODEL`, default `claude-sonnet-5`) to use Claude instead — better output quality, but a paid API. Results are cached on the lead; pass `?force=true` to regenerate.
